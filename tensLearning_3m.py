...
class RandomHillClimber(LearningMethod):
    ...
    def run_experiment(self, time=30):
        ...
        # self.tens.start_experiment(verbose=True)
...
class BayesianOptimizer(LearningMethod):
    ...
    def run_optimizer(self, iterations=40):
        ...
        param = {
            ...
            'z': ('int', [MIN_SPEED, MAX_SPEED])
        }

        meta = lambda x, y, z: self.experiment(int(x), int(y), int(z))
...
def main(method):
    ...
    BO = BayesianOptimizer([VALTR5, VALTR7, VALTR8], tracking_method=method)
